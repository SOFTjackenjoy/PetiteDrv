...
typedef struct NllsTimer_s{
    ...
    softtimer_cb_t cb;
    /* start */
    SOFTTIMER_V debtime;//定时器剩余时间
    SOFTTIMER_V base;   //上一个判定时间
    SOFTTIMER_V period;//定时器定时周期
...
{    
    ...
    if (back_type == SOFTTIMER_TYPE_NO_LOOP) {
        /** 使用简单的CAS机制，将定时器加入链表 */
        while(1) {
            /*step 1：等ploop->end指向的点的next为空
                如果不为空，说明其他人正在执行step:2-3，还没到4 */
            if (ploop->end->next != 0){
                osDelay(5);
                continue;
            } 
            /* step 2：将定时器加入最后一个点的next指针 
...
int slst_loop(void *loop)
{
    ...
    while(1) {
        ...
        switch(ptimer->type) {
            ...
            case SOFTTIMER_TYPE_PERIODIC:
                /* 必须每次都获取系统时间，以便避免：
                    In timer_a callback start timer_b,
                    timer_b->base > systime, 
                    pasttime big! 
                    run timer_b callback ----这不合逻辑
                    */
                systime = Stime_get_systime();
                /* 计算定时器已经过去的时间 */
    ...
    #if 1
    /** 统计链表中最快到达的定时器时间
     * 返回到外部，以便进入休眠，释放CPU时间
     * 
     * 如果loop任务不进入sleep，可以不统计，随便返回一个值即可。
     */
