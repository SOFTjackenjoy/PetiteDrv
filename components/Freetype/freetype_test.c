...
void WIN_FontInit (void)
{
	...
	if (ft_error) {
		...
		/*
			chuheisong.ttf
			*/
		ft_error=FT_New_Face (g_ft_library, "/0:/font/chuheisong.ttf", 0, &g_ft_face);
...
int freetype_getdata(uint16_t pixw, int16_t pixh, struct _strBitmapHeadT* head, uint8_t *dotbuf, uint16_t unicode)
{
	unsigned long foffset=0; 
...
int freetype_showstr_unicode(uint16_t pixw, int16_t pixh, DevLcdNode *lcd, uint16_t x, uint16_t y, uint16_t *unicodestr)
{
	...
	uint8_t dotdata;
	struct _strBitmapHeadT BitmapHead;
